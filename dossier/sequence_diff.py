...
Reuses the J2 classifier (_align_proteins, vendored in _protein_align.py from
junior_surveyor.j2_protein_diff) for the protein_change_type / summary stats
so the dossier never disagrees with hits_deep.csv. align_segments() is a
SEPARATE alignment, deliberately not edlib: edlib's unit-cost edit distance
has no preference for consolidating gaps, so a single 675-aa deletion can
come back as ~150 scattered 1-2 residue fragments (any distribution of the
same total gap length costs the same). That's fine for J2's summary stats
(position of first/last change, counts) but renders as a nonsense fan of
ribbons. Biopython's PairwiseAligner with an affine gap penalty (steep open,
shallow extend) prefers few large gaps over many small ones -- a
biologically sane shape for the ribbon diagram.
"""
...
from dossier._protein_align import _align_proteins
